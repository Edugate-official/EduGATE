...
public:
	// FileList(const TGWindow *p,const TGWindow *pmain,TCanvas *can,UInt_t w,UInt_t h);
...
//----------------------------------------------------------------------------------
	...
	// third tile (text)
		...
		for (line_number =1; line_number <11; line_number++)
			{   if (line_number == 1)
			    	{text = "- particle: ";
			        if (array[5] == "gamma")
			        	array[5] = "#gamma";
			        text += array[5];}
			    if ((line_number == 10) && (array[5] == "e+"))
			        {text = "- positron annihilation: ";
			    		if (array[7] == "iso")
			    			{
			    			if (array[11] == "disabled")
			        			{text2 = "disabled";
			        			text += text2;}
			        		if (array[11] == "enabled")
			         			{text2 = "enabled";
