...
{	
	...
	for (int i=0; i<numCombo; i++) {
		...
		if (fCombo[i]->GetSelected()!=-1) {
			...
			//string s_tmp( ((TGTextLBEntry*)entry)->GetText()->GetString());
			//value = new char[ s_tmp.length() +1 ];							
			//strcpy( value, s_tmp.c_str());	
...
{	
	...
	for (int i=0; i<numCombo; i++) {
		...
		const char *value =((TGTextLBEntry*)entry)->GetText()->GetString();
		//string s_tmp( ((TGTextLBEntry*)entry)->GetText()->GetString());
		//char* value = new char[ s_tmp.length() +1 ];	
		//strcpy( value, s_tmp.c_str());
...
//		fCombo[i]->EnableTextInput(true);
   ...
   SetWindowName("EduGATE Configuration Menue");
   MapSubwindows();
...
void GenerateGateConfiguration(const char *conf)
{
   // Popup the GUI...
